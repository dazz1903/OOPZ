...
type VsRow = { playerName?: string; points?: number; rank?: number; daily?: { mon?: number|null; tue?: number|null; wed?: number|null; thu?: number|null; fri?: number|null; sat?: number|null } };
type ArenaRow = { playerName?: string; score?: number; squadPower?: string|number; rank?: number };
type HeroPowerRow = { playerName?: string; heroPower?: number; serverRank?: number };
...
function playerName(value: string | undefined) {
  return String(value ?? '').replace(/\d\/4$/, '').replace(/\s+/g, ' ').trim();
}
...
export async function POST(request: Request) {
  ...
  const body = contentType.includes('application/json')
    ? await request.json().catch(() => null) as { rows?: LwmaRow[]; vsRows?: VsRow[]; weekStart?: string; arenaRows?: ArenaRow[]; heroPowerRows?: HeroPowerRow[]; captureDate?: string } | null
  ...
  if (vsRows.length && weekStart) {
    await env.DB.batch(vsRows.map((row) => env.DB.prepare(`INSERT INTO vs_scores (week_start, player_name, points, rank, mon_points, tue_points, wed_points, thu_points, fri_points, sat_points, captured_at)
      ...
      .bind(weekStart, playerName(row.playerName), Math.max(0, Math.round(row.points ?? 0)), Math.max(1, Math.round(row.rank ?? 1)), row.daily?.mon ?? null, row.daily?.tue ?? null, row.daily?.wed ?? null, row.daily?.thu ?? null, row.daily?.fri ?? null, row.daily?.sat ?? null, capturedAt)));
  ...
  const arenaRows = (body?.arenaRows ?? []).filter((row) => row.playerName && Number.isFinite(row.score) && Number.isFinite(row.rank)).slice(0, 200);
  const heroPowerRows = (body?.heroPowerRows ?? []).filter((row) => row.playerName && Number.isFinite(row.heroPower) && Number.isFinite(row.serverRank)).slice(0, 200);
  if (captureDate && arenaRows.length) await env.DB.batch(arenaRows.map((row) => env.DB.prepare(`INSERT INTO arena_rankings (capture_date, player_name, score, squad_power, server_rank, captured_at)
    ...
    .bind(captureDate,playerName(row.playerName),Math.round(row.score ?? 0),numberValue(row.squadPower),Math.round(row.rank ?? 0),capturedAt)));
  if (captureDate && heroPowerRows.length) await env.DB.batch(heroPowerRows.map((row) => env.DB.prepare(`INSERT INTO hero_power_rankings (capture_date, player_name, hero_power, server_rank, captured_at)
    ...
    .bind(captureDate,playerName(row.playerName),Math.round(row.heroPower ?? 0),Math.round(row.serverRank ?? 0),capturedAt)));
  return NextResponse.json({ ok: true, imported: rows.length, vsImported: vsRows.length, arenaImported: arenaRows.length, heroPowerImported: heroPowerRows.length });
}
