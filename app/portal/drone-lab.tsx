  ...
  const componentPlans = useMemo(
    () =>
      COMPONENTS.map((component, i) => {
        ...
        const boxes = deficit
          ? Math.ceil(Math.ceil(deficit / chestValue) / component.chance)
          : 0;
        return {
          ...component,
          equipped,
    ...
    [p.components, p.componentInventory, p.chestLevel],
  );
        ...
        <section className="rounded-3xl border border-[#252c3d] bg-[#0f1420] p-6">
          ...
          <div className="mt-7 flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-xs font-black text-[#59d7ff]">
                EQUIPPED COMPONENTS & INVENTORY
              </p>
              <p className="mt-1 text-xs text-[#68728b]">
                Inventory columns are component levels 1–6. Three matching
                components merge into the next level.
              </p>
            </div>
            <div className="min-w-52">
              <p className="text-[10px] font-black text-[#7f899f]">
                CALCULATE USING CHEST LEVEL
              </p>
          ...
          <div className="mt-4 space-y-3">
            {componentPlans.map((plan, i) => (
              ...
              >
                <div className="grid gap-4 lg:grid-cols-[1fr_150px_1.6fr_150px] lg:items-end">
                  <div>
                      ...
                      {p.componentInventory[i].map((count, level) => (
                        <label key={level} className="text-center">
                          ...
                          <input
                            ...
                            className="mt-1 w-full rounded-lg border border-[#344153] bg-[#0c121b] px-1 py-2 text-center font-black text-white outline-none"
                          />
                        </label>
                      ))}
                    </div>
