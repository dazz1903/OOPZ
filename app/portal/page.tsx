...
import { getMemberAccess, listCurrentVsScores, listPlayers, syncMember } from '@/lib/member-access';
...
export default async function PortalPage() {
  ...
  const [access, players, vsScores] = await Promise.all([getMemberAccess(session.discordId), listPlayers(), listCurrentVsScores()]);
  ...
  return <Portal member={session} access={access} players={players} vsScores={vsScores} pendingClaims={pendingClaims} />;
}
