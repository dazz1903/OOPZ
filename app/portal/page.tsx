...
export default async function PortalPage() {
  ...
  const [access, players, vsScores, arena, heroPower] = await Promise.all([getMemberAccess(session.discordId), listPlayers(), listCurrentVsScores(), listCurrentArena(), listCurrentHeroPower()]);
  ...
  return <Portal member={session} access={access} players={players} vsScores={vsScores} arena={arena} heroPower={heroPower} pendingClaims={pendingClaims} />;
}
