...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || state !== savedState || !verifier)
    return failed(request, "invalid-oauth-state");
  if (!clientId || !clientSecret || !guildId || !process.env.AUTH_SECRET)
    return failed(request, "discord-not-configured");
  ...
  if (!userResponse.ok || !memberResponse.ok)
    return failed(request, "not-an-oopz-member");
  ...
  const allowedRoles = (process.env.DISCORD_ALLOWED_ROLE_IDS ?? "")
    .split(",")
    .map((role) => role.trim())
    .filter(Boolean);
  const preapprovedObserver = (await getMemberAccess(user.id)).isObserver;
  if (
    !preapprovedObserver &&
    allowedRoles.length &&
    !member.roles.some((role) => allowedRoles.includes(role))
  )
    return failed(request, "missing-discord-role");
